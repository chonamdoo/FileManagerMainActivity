...
//获取文件信息
    private FileInfo getFileInfo(Cursor cursor) {
        return (cursor == null || cursor.getCount() == 0) ? null : Util.getFileInfo(cursor
                .getString(cursor
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (fileInfo == null) {
            ...
            fileInfo.modifiedDate = cursor
                    .getLong(cursor.getColumnIndex(FileColumns.DATE_MODIFIED));
        }

        if (fileInfo != null) {
            Util.setViewText(view, R.id.file_name, fileInfo.fileName);
            if (mFileSettingsHelper
                    .getBoolean(FileSettingsHelper.KEY_ONLY_SHOW_FILENAME, false)) {
                ...
                Util.setViewText(view, R.id.file_count, fileInfo.isDir ? "(" + fileInfo.count
                        + ")"
                        : "");
                Util.setViewText(view, R.id.modified_time,
                        Util.formatDateString(context, fileInfo.modifiedDate));
                Util.setViewText(view, R.id.file_size,
            ...
            view.findViewById(R.id.file_checkbox_area).setOnClickListener(
                    new FileOperationHelper.FileOnOperationClickListener(context,
